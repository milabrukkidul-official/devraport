...
async function loadSiswa() {
  try {
    const data = await API.call('getSiswa');
...
function renderTabelSiswa(list) {
  ...
  list.forEach((s, i) => {
    ...
    tr.innerHTML = `
      ...
      <td>${s.pesan || ''}</td>
      <td>
        <button class="btn-warning" onclick="editSiswa(${i})" style="padding:4px 8px;font-size:0.78rem;">✏️</button>
        <button class="btn-danger"  onclick="hapusSiswa(${i})" style="padding:4px 8px;font-size:0.78rem;margin-left:4px;">🗑️</button>
      </td>`;
...
async function simpanSiswa() {
  ...
  try {
    await API.post('saveSiswa', { siswa: JSON.stringify(siswa), rowIndex: idx });
...
async function hapusSiswa(idx) {
  ...
  try {
    await API.post('deleteSiswa', { rowIndex: idx });
